            ...
            <div class="tile">
               ...
               Add Quiz
                ...
                <form action="{{ route('admin.quiz.store') }}" method="POST" role="form" enctype="multipart/form-data">
                    ...
                    <div class="tile-body">
                        <div class="form-group">
                            <label class="control-label" for="quiz_question_id">Quiz <span class="m-l-5 text-danger"> *</span></label>
                            <select class="form-control @error('quiz_question_id') is-invalid @enderror"
                                name="quiz_question_id" id="quiz_question_id" value="{{ old('quiz_question_id') }}">
                                <option selected disabled>Select one</option>
                                @foreach ($quizQuestions as $quizQuestion)
                                    <option value="{{ $quizQuestion->id }}">{{ $quizQuestion->question ? $quizQuestion->question :''}}</option>
                                @endforeach
                            </select>
